...
var WebSocketClass = require('ws');

function _interopDefaultLegacy (e) { return e && typeof e === 'object' && 'default' in e ? e : { 'default': e }; }

var WebSocketClass__default = /*#__PURE__*/_interopDefaultLegacy(WebSocketClass);
...
class ClientRequest {
	...
	send() {
		...
		const asyncRequest = Acid.promise((accept) => {
...
class ClientSocket {
	 ...
	 */
	process(response) {
		const compiledResponse = Acid.jsonParse(response);
	...
	reconnect() {
		...
		const context = this;
		const { socket } = this;
		if (!Acid.hasValue(context.connectInterval)) {
			socket.onopen = null;
	...
	onopen(context, accept) {
		...
		context.socket.onmessage = (socketEvent) => {
			...
			Acid.apply(context.process, context, [socketEvent.data]);
		};
		context.socket.onclose = () => {
			console.log('close', context.connectInterval, !Acid.hasValue(context.connectInterval));
			if (!Acid.hasValue(context.connectInterval)) {
	...
	connect() {
		const context = this;
		return Acid.promise((accept) => {
			context.socket = Acid.construct(WebSocketClass__default["default"], [context.hostname]);
	...
	send(data) {
		if (this.app.config.debug) {
			console.log(Acid.stringify(data, null, ' '));
		}
		if (this.socket.readyState === 1) {
			if (Acid.isPlainObject(data)) {
				this.socket.send(Acid.stringify(data));
			} else if (Acid.isString(data)) {
...
class Watcher {
	...
	stop() {
		if (Acid.hasValue(this.id)) {
			Acid.drop(this.container(), this.id);
			this.id = null;
			this.active = false;
		}
	}
...
class SoloNFT {
	...
	constructor(config) {
		this.state = 1;
		console.log(config);
		Acid.assign(this.config, config);
		if (config.debug) {
			Watcher.debug = true;
		}
		return this.connect();
	...
	async connect() {
		...
		this.socket = await Acid.construct(ClientSocket, [this, this.config]);
		this.state = 2;
...
async function soloNFT(config) {
	const clientSDK = await Acid.construct(SoloNFT, [config]);
...
exports.SoloNFT = SoloNFT;
exports.Watcher = Watcher;
exports["default"] = soloNFT;
exports.soloNFT = soloNFT;
exports.watch = watch;
